...
class Signal():
    ...
    def get_F_0( self, min_pitch = 75, max_pitch = 550, max_num_candidates = 2, octave_cost = .01, 
        ...
        ::
            ...
            sig.get_F_0()
            #TODO: might need to change this code block once code is changed from class to functions
        ...
        #filtering by Nyquist Frequency 
    ...
    def get_HNR( self, min_pitch = 90, silence_threshold = .01, ):   
        ...
        Raises:
            TypeError: min_pitch must be an int.
            ValueError: The minimum pitch cannot be equal or less than zero.
        ...
        if type(min_pitch) != int :
            raise TypeError( "min_pitch must be an int." )
        if min_pitch <= 0:
            raise ValueError( "The minimum pitch cannot be equal or less than zero." )
        ...
        for index in range( len( segmented_signal ) ):
            ...
            if std_ratio < 1.3 and local_peak / global_peak > .05 :
                ...
                For each segment we follow the given algorithm (steps 3.2-3.10), by
                1. Subtracting the mean of the segment
                2. Multiply the segment by the hanning window (we use hamming to prevent errors
                ...
                #eliminating points in the autocorrelation that are not finite (caused by dividing by 
                #a number close to zero)
                r_x = r_x[ np.isfinite( r_x ) ]
                r_len = int( len( r_x ) / 2 ) 
        ...
        #TODO: we will want to test to make sure best_cands is not empty, returning some appropriate value if it is.                
        best_cands = max( best_cands )
        return 10 * np.log10( best_cands / ( 1 - best_cands ) )

        #TODO:change from class to functions
        #TODO: change unit tests to match
